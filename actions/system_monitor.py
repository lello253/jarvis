...
def _get_cpu_temp() -> float:
    ...
    if _OS == "Windows":
        try:
            ...
            if tz and hasattr(tz[0], "CurrentTemperature"):
    ...
    def check(self) -> str | None:
        ...
        if cpu >= self.thresholds["cpu"]:
            ...
            if self._cpu_streak >= _CPU_STREAK and self._can_alert("cpu"):
                alerts.append(
                    f"[SYSTEM_ALERT] L'utilizzo della CPU è molto elevato ({cpu:.0f}%) "
                    "da diversi secondi. Avvisa l'utente in italiano e suggerisci di "
                    "chiudere i processi più pesanti."
                )
                self._record("cpu")
                self._cpu_streak = 0
        else:
        ...
        if ram >= self.thresholds["ram"] and self._can_alert("ram"):
            alerts.append(
                f"[SYSTEM_ALERT] La memoria RAM è al {ram:.0f}% — quasi esaurita. "
                "Avvisa l'utente in italiano e suggerisci di liberare risorse."
            )
            self._record("ram")

        if temp > 0 and temp >= self.thresholds["temp"] and self._can_alert("temp"):
            alerts.append(
                f"[SYSTEM_ALERT] La temperatura della CPU è di {temp:.0f}°C — oltre il limite di sicurezza. "
                "Avvisa l'utente in italiano e consiglia di ridurre il carico di lavoro."
            )
            self._record("temp")

        if gpu >= 0 and gpu >= self.thresholds["gpu"] and self._can_alert("gpu"):
            alerts.append(
                f"[SYSTEM_ALERT] Il carico della GPU è al {gpu:.0f}%. "
                "Informa brevemente l'utente in italiano."
            )
            self._record("gpu")
