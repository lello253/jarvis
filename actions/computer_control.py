...
def _random_data(data_type: str) -> str:
    ...
    if dt == "phone":     return f"+393{random.randint(10,99)}{random.randint(1000000, 9999999)}"
    if dt == "birthday":  return f"{random.randint(1, 28):02d}/{random.randint(1, 12):02d}/{random.randint(1980, 2000)}"
    if dt == "address":   return f"Via {random.choice(['Roma', 'Milano', 'Garibaldi', 'Dante', 'Mazzini'])} {random.randint(1, 150)}"
    if dt == "zip_code":  return str(random.randint(10000, 99999))
    if dt == "city":      return random.choice(["Roma", "Milano", "Torino", "Napoli", "Bologna"])
...
def _type(text: str, interval: float = 0.03) -> str:
    """Digitazione sicura tramite Appunti per gestire caratteri speciali/accentati."""
    _require_pyautogui()
    time.sleep(0.2)
    
    # Se contiene caratteri speciali o accentati usiamo gli appunti
    if _PYPERCLIP and any(ord(c) > 127 for c in text):
...
def _hotkey(*keys) -> str:
    _require_pyautogui()
    clean_keys = [k.strip().lower() for k in keys if k]
    pyautogui.hotkey(*clean_keys)
    return f"Hotkey: {'+'.join(clean_keys)}"
...
def _screen_find(description: str) -> tuple[int, int] | None:
    ...
    try:
        ...
        prompt = (
            ...
            f"Reply with ONLY the center coordinates formatted as: x,y "
        ...
        # Regex flessibile per estrarre le coordinate x,y anche se Gemini aggiunge formattazioni
