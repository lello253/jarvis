...
class _BrowserSession:
    ...
    def run(self, coro, timeout: int = 15) -> str:
        if not self._loop:
    ...
    async def _launch(self):
        ...
        if self._spec is None:
            raise RuntimeError(
                f"'{self.browser_name}' non è supportato su {_OS}."
            )
    ...
    async def go_to(self, url: str) -> str:
        ...
        async def _do_goto(p: Page) -> str:
            try:
                await p.goto(url, wait_until="commit", timeout=15_000)
    ...
    async def back(self) -> str:
        page = await self._get_page()
        try:
            await page.go_back(timeout=5_000)
    ...
    async def forward(self) -> str:
        page = await self._get_page()
        try:
            await page.go_forward(timeout=5_000)
...
def browser_control(
            ...
            try:
                if action == "search":
                    result = sess.run(sess.search(params.get("query", ""),
                                                  params.get("engine", "google")), timeout=15)
                elif action == "new_tab":
                    result = sess.run(sess.new_tab(params.get("url", "")), timeout=15)
                else:
                    result = sess.run(sess.go_to(params.get("url", "")), timeout=15)
            except concurrent.futures.TimeoutError:
                result = f"Browser action '{action}' timed out (15s)."
            except Exception as e:
    ...
    try:
        ...
        if last:
            try:
                sess.run(sess.go_to(last), timeout=15)
            except Exception as e:
                print(f"[Browser] Could not resume last page ({last}): {e}")

        if action == "click":
            result = sess.run(sess.click(params.get("selector"), params.get("text")), timeout=15)
        elif action == "type":
            result = sess.run(sess.type_text(
                params.get("selector"), params.get("text", ""), params.get("clear_first", True)), timeout=15)
        elif action == "scroll":
            result = sess.run(sess.scroll(params.get("direction", "down"), int(params.get("amount", 500))), timeout=15)
        elif action == "fill_form":
            result = sess.run(sess.fill_form(params.get("fields", {})), timeout=15)
        elif action == "smart_click":
            result = sess.run(sess.smart_click(params.get("description", "")), timeout=15)
        elif action == "smart_type":
            result = sess.run(sess.smart_type(params.get("description", ""), params.get("text", "")), timeout=15)
        elif action == "get_text":
            result = sess.run(sess.get_text(), timeout=15)
        elif action == "get_url":
            result = sess.run(sess.get_url(), timeout=15)
        elif action == "press":
            result = sess.run(sess.press(params.get("key", "Enter")), timeout=15)
        elif action == "close_tab":
            result = sess.run(sess.close_tab(), timeout=15)
        elif action == "screenshot":
            result = sess.run(sess.screenshot(params.get("path")), timeout=15)
        elif action == "back":
            result = sess.run(sess.back(), timeout=15)
        elif action == "forward":
            result = sess.run(sess.forward(), timeout=15)
        elif action == "reload":
            result = sess.run(sess.reload(), timeout=15)
        else:
